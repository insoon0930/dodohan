import 'package:flutter/material.dart';

class ThemeColors {
  static const Color main = Color(0xff794ff1);
  static const Color mainLight = Color(0xffa183f5);
  static const Color mainLightest = Color(0xffe5dffd);
  static const Color mainLightest2 = Color(0xffefebff);

  static const Color grayDark = Color(0xff89898a);
  static const Color grayText = Color(0xffB1B4B7);
  static const Color grayLightest = Color(0xffeef0f3);
  static const Color cream =  Color(0xfff7f7f7);

  static const Color redLight = Color(0xfffd354f);
  static const Color blueLight = Color(0xff3585fd);
  static const Color yellowLight = Color(0xfffdd235);
  static const Color orangeLight = Color(0xfffdb435);
  static const Color greenLight = Color(0xff35fd49);
  static const Color pinkLight = Color(0xffd835fd);
}
